...
\begin{itemize}
 ...
 \item Checkout: repo ‚Üí working copy
 \item Commit: working copy ‚Üí repo
...
\begin{itemize}
 ...
 \item Fr√ºhes Identifizieren von Problemen
...
\item ÑYou can't manage what you can't control, and you can't control what you don't measureî
...
\begin{description}
	\item[Too Much Code Smells] \hfill
		...
		\end{itemize}
	\item[Not enoughù Code Smells] \hfill 
		\begin{itemize}
...
\begin{itemize}
	...
	\item Klasse: Erkl\"{a}rt Eigenschaft verwendete Algorithmen (Performance, Speicher, Ö)
...
\begin{itemize}
\item Keine Ñmagischen Zahlenì wie 4.352, viel besser Konstanten verwenden
...
\begin{itemize}
	...
	\item[\Bold {WARN}] Informiert \"{u}ber Ñschlimmesì Ereignis
...
\item FileAppender ñ Write to a log file
\item SocketAppender ñ Dumps log output to a socket
\item SyslogAppender ñ Write to the syslog.
\item NTEventLogAppender ñ Write the logs to the NT Event Log system.
\item RollingFileAppender ñ After a certain size is reached it will rename the old file and start with a new one.
\item SocketAppender ñ Dumps log output to a socket
\item SMTPAppender ñ Send Messages to email
\item JMSAppender ñ Sends messages using Java Messaging Service
\end{itemize}
...
\item[\Bold{\%m:}] Message
\item[\Bold{\%M:}] Methode
\item[\Bold{\%n:}] Neue Linie wie Ñ$\backslash$nì
...
\item Nicht zu Ñtiefì testen, wie z.B. Verhalten von Standardkomponenten
...
\item Wenig(er) Kontrolle √ºber die Laufzeitumgebung
\item Browser-Inkompatibilit\"aten (IE vs. Firefox vs. Chrome vs. ‚Ä¶)
\item Tools m√ºssen auf verschiedene Browser zugreifen k\"onnen
...
\begin{itemize}
\item \textbf{\Bold Selenium IDE:} ein Capture/Replay-Plugin f√ºr Firefox erlaubt auch das Editieren der aufgenommenen Skripts
\item \textbf{\Bold Selenium WebDriver:} API um Browser programmatisch anzusteuern (z.B. aus Java, C-Sharp, PHP, Ruby‚Ä¶). Kann auch aufgenommene Skripts gegen andere Browser (‚â  Firefox) ausf√ºhren
...
F√ºr den Kunden stehen die Gesch\"aftsprozesse immer im Vordergrund.\\\\
...
\item Wiederholte Durchf√ºhrung
\item Unabh\"angig von GUI
\end{itemize}

\subsubsection{Feedback}
Ein fr√ºhes Feedback gegen√ºber dem Kunden, hilft eine Software korrekt und wunschgem\"ass zu entwickeln.\\\\
\textbf{\Bold Problem:} Unittests sind nicht wirklich aussagekr\"aftig und GUI-Test werden erst sp\"at im Projekt gemacht. Daher sollten fr√ºh im Projekt mit dem Kunden zusammen Akzeptanztest erstellt und definiert werden.
...
\Bold {Konzepte:}
\begin{itemize}
\item Ausf\"uhrbare Use Cases und Workflowsú
\item F\"ur Kunden/Benutzer verst\"andlich und durch diesen erstellbar
\item Konkrete Beispiele (Testing by Exampleú)
\end{itemize}
...
Statt Fixtures.TestSearchSubmit$ \rightarrow$ ûTest Search Submit
...
Die Testf\"alle werden bei Fit in HTML-Tabellen definiert. Die Anbindung an das zu testende Programm erfolgt √ºber Java-Klassen, die "Fixtures" genannt werden, und die bestimmte Fit-Fixture-Klassen erweitern.\\
Es gibt die drei Basis-Fixtures ColumnFixture, RowFixture und ActionFixture. Wenn auf Zusatzbibliotheken (z.B. FitLibrary) zur√ºckgegriffen wird, stehen zus\"atzlich noch viele weitere Fixtures zur Verf√ºgung (z.B. DoFixture).\\
...
\item Prozesskontext definieren
	...
	\end{itemize}
\item Fit Fixtures f\"ur dom\"anenspezifische Bed\"urfnisse verf\"ugbar (z.B. Web, DB‚Ä¶)

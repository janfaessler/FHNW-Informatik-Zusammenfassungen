...
>>>>>>> Revert "language update from mark"
...
\begin{itemize}
 ...
 \item Checkout: repo â†’ working copy
 \item Commit: working copy â†’ repo
...
<<<<<<< HEAD
\begin{description}
	\item[Complete] Alle dazugehörigen Ressourcen inbegriffen
	\item[Repeatable] Beliebig vielmal wiederholbar
	\item[Informative] Stellt wichtige Informationen bereit
	\item[Schedulable] Komplett und wiederholbar
	\item[Portable] Unabhängig von Maschine
\end{description}
=======
\begin{itemize}
...
=======
 \item FrÃ¼hes Identifizieren von Problemen
...
<<<<<<< HEAD
\begin{description}
 \item[Validation] 			Ist das Produkt überhaupt das, was gewünscht wurde? 
 \item[Verifikation]			Ist das Produkt korrekt programmiert?
 \item[Regression]				Iteratives Wiederholen von Tests 
 \item[Software Fault]		Bug im Code
 \item[Software Error]		Zustand der beim Ausführen von verbuggtem Code auftritt (z.B. nicht abgefangene Exception)
 \item[Software Failure] Software verhält sich nicht wie erwartet (Requirements nicht erfüllt oder Bugs)
\end{description}
=======
\begin{itemize}
...
<<<<<<< HEAD
\begin{description}
	\item[Automatic]		Tests ausführen und Resultate überprüfen
	\item[Thorough] 		Alles was Fehlschlagen kann testen $\Ra$ Code Coverage
	\item[Repeatable] 	Der Test ist beliebig oft wiederholbar; wird am Code nichts geändert, bleibt das Resultat dasselbe
	\item[Independent]	Tests hängen nicht voneinander ab
	\item[Professional] Gleichen professionellen Standard wie für den Code benutzen
\end{description}
...
<<<<<<< HEAD
\begin{description}
	\item[Dummy] Meist nur benutzt um Parameter-Listen zu füllen 
	\item[Stubs] Minimale Implementation von Schnittstellen oder Basisklassen, void hat normalerweise kein Code und die anderen Methoden fest kodierte Werte
	\item[Spys]	 	Ähnlich wie Stubs, aber zeichnen verwendete Mitglieder von Klasse auf
	\item[Fakes]	Haben bereits oft komplexe Implementierung, verwalten Interaktion zwischen den verschiedenen Mitgliedern von denen Sie erben
	\item[Mocks] 	Vorprogrammierte Objekte mit fest kodierten Erwartungen
\end{description}
=======
\begin{itemize}
...
\item ï¿½You can't manage what you can't control, and you can't control what you don't measureï¿½
...
\begin{description}
	\item[Too Much Code Smells] \hfill
		...
		\end{itemize}
	\item[Not enoughï¿½ Code Smells] \hfill 
		\begin{itemize}
...
\begin{itemize}
	...
	\item Klasse: Erkl\"{a}rt Eigenschaft verwendete Algorithmen (Performance, Speicher, ï¿½)
...
=======
\item Keine ï¿½magischen Zahlenï¿½ wie 4.352, viel besser Konstanten verwenden
...
\subsection{Konzept}
\begin{description}
	\item[Priority] Bestimmt, was geloggt wird.
	\item[Level] Bestimmt, ob etwas geloggt wird.
	\item[Appender] Bestimmt, wo etwas geloggt wird.
	\item[Layout] Bestimmt, wie das Layout aussieht.
	\item[Logger] Schreibt einen Log-Eintrag zu einem Appender in einem bestimmten Layout falls das Logging-Level angemessen ist.
\end{description}

\subsection{Logging Levels}
\begin{description}
	\item[FATAL] Fehler, bei dem die Applikation sich nicht mehr erholen kann
	\item[ERROR] Fehler, die Applikation ist jedoch nicht lauffähig
	\item[WARN] Informiert über 'schlimmes' Ereignis
	\item[INFO] Informiert z.B. über Fortschritt der Applikation
	\item[DEBUG] Stellt Informationen für Debug bereit 
	\item[TRACE] Sehr detaillierte Informationen um Informationsfluss zu verfolgen
=======
...
\begin{itemize}
	...
	\item[\Bold {WARN}] Informiert \"{u}ber ï¿½schlimmesï¿½ Ereignis
...
\item FileAppender ï¿½ Write to a log file
\item SocketAppender ï¿½ Dumps log output to a socket
\item SyslogAppender ï¿½ Write to the syslog.
\item NTEventLogAppender ï¿½ Write the logs to the NT Event Log system.
\item RollingFileAppender ï¿½ After a certain size is reached it will rename the old file and start with a new one.
\item SocketAppender ï¿½ Dumps log output to a socket
\item SMTPAppender ï¿½ Send Messages to email
\item JMSAppender ï¿½ Sends messages using Java Messaging Service
\end{itemize}

\subsection{PatternLayout Platzhalter}
<<<<<<< HEAD
\begin{description}
\item[\%C] For example, for the class name org.apache.xyz.SomeClass, the pattern \%C{1} will output SomeClass
\item[\%d] For example, \%d\{HH:mm:ss,SSS\} or \%d\{dd MMM yyyy HH:mm:ss,SSS\}
\item[\%F] Datei, die das Logging veranlasst hat
\item[\%L] Code-Linie vom Logevent
\item[\%m] Message
\item[\%M] Methode
\item[\%n] Neue Linie wie '$\backslash$n'
\item[\%p] Priorität
\item[\%r] Zeitmessung Logevent
\item[\%t] Thread
\end{description}
=======
\begin{itemize}
...
\item[\Bold{\%m:}] Message
\item[\Bold{\%M:}] Methode
\item[\Bold{\%n:}] Neue Linie wie ï¿½$\backslash$nï¿½
...
\item Nicht zu ï¿½tiefï¿½ testen, wie z.B. Verhalten von Standardkomponenten
...
=======
\item Wenig(er) Kontrolle Ã¼ber die Laufzeitumgebung
\item Browser-Inkompatibilit\"aten (IE vs. Firefox vs. Chrome vs. â€¦)
\item Tools mÃ¼ssen auf verschiedene Browser zugreifen k\"onnen
>>>>>>> Revert "language update from mark"
...
\begin{itemize}
\item \textbf{\Bold Selenium IDE:} ein Capture/Replay-Plugin fÃ¼r Firefox erlaubt auch das Editieren der aufgenommenen Skripts
\item \textbf{\Bold Selenium WebDriver:} API um Browser programmatisch anzusteuern (z.B. aus Java, C-Sharp, PHP, Rubyâ€¦). Kann auch aufgenommene Skripts gegen andere Browser (ï¿½ï¿½  Firefox) ausfÃ¼hren
>>>>>>> Revert "language update from mark"
...
\item[Infrastruktur ist nicht verfügbar]
\item[Automatisierung] \hfill \\ 
	Essentiell für Regressionstests\end{description}
=======
FÃ¼r den Kunden stehen die Gesch\"aftsprozesse immer im Vordergrund.\\\\
...
\Bold {Konzepte:}
\begin{itemize}
\item Ausf\"uhrbare Use Cases und Workflowsï¿½
\item F\"ur Kunden/Benutzer verst\"andlich und durch diesen erstellbar
\item Konkrete Beispiele (Testing by Exampleï¿½)
>>>>>>> Revert "language update from mark"
\end{itemize}
...
Statt Fixtures.TestSearchSubmit$ \rightarrow$ ï¿½Test Search Submit
...
=======
Die Testf\"alle werden bei Fit in HTML-Tabellen definiert. Die Anbindung an das zu testende Programm erfolgt Ã¼ber Java-Klassen, die "Fixtures" genannt werden, und die bestimmte Fit-Fixture-Klassen erweitern.\\
Es gibt die drei Basis-Fixtures ColumnFixture, RowFixture und ActionFixture. Wenn auf Zusatzbibliotheken (z.B. FitLibrary) zurÃ¼ckgegriffen wird, stehen zus\"atzlich noch viele weitere Fixtures zur VerfÃ¼gung (z.B. DoFixture).\\
>>>>>>> Revert "language update from mark"
...
=======
	...
	\end{itemize}
\item Fit Fixtures f\"ur dom\"anenspezifische Bed\"urfnisse verf\"ugbar (z.B. Web, DBâ€¦)
>>>>>>> Revert "language update from mark"

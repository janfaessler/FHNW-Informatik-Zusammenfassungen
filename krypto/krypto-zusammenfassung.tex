...
\tikzstyle{decision} = [diamond, draw, fill=blue!20,
   text width=5.8em, text badly centered, node distance=2.5cm, inner sep=0pt]
...
\begin{tikzpicture}[scale=2, node distance = 3.5cm, auto]
   ...
   \node [decision, below of=init] (quadrat) {Ist $t^2-n$ eine Quadratzahl?};
   \node [block, left of=quadrat] (nein) {$t:=t+1$};
   \node [block, right of=quadrat] (ja) {$s:=\sqrt{t^2-n}$\\$a:=t+s$\\$b:=t-s$};

   % Draw edges
   \path [line] (init) -- (quadrat);
   \path [line] (quadrat) -- node [, color=black] {nein} (nein);
   \path [line] (quadrat) -- node [, color=black] {ja} (ja);
   \path [line] (nein) |- (init);
\end{tikzpicture} \\ \\

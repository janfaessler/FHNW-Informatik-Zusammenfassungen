...
\begin{description}
	...
	\item[Berechnung der WP] \hfill 
		\begin{itemize}
			...
			\item Anweisung für Anweisung r\"uckwärts rechnen
			\item Berechnungsregeln für Zuweisung, Sequenz, Selektion, Iteration
		\end{itemize}
\end{description}
\subsubsection{WP-Rechenregel: Zuweisung}
\begin{description}
	\item[Gegeben] \hfill \\
		Zuweisung $P = (x' := y)$ \\
		Nachbedingung S
...
\end{description}
\subsubsection{WP-Rechenregel: Sequenz}
\begin{description}
	\item[Gegeben] \hfill \\
...
wp((Anweisung_1; Anweisung_2; ...; Anweisung_{n-1}), wp(Anweisung_n, S))$
	\item[Beispiele] \hfill \\
		...
		$= (y = b \wedge x = a)$
\end{description}
\subsubsection{WP-Rechenregel: Selektion}
\begin{description}
	\item[Gegeben] \hfill \\
	...
	\item[Regel] \hfill \\
		...
		$= \neg b \wedge b \vee \neg b \wedge wp(P_2,S) \vee  b \wedge wp(P_1,S) \vee wp(P_1,S)\wedge wp(P_2,S) $ \\
		$= false \vee \neg b \wedge wp(P_2,S) \vee b \wedge wp(P_1,S) \vee false$ \\
		$=\textcolor{blue}{b \wedge wp(P_1,S)\vee \neg b \wedge wp(P_2,S)}$
\end{description}

\subsubsection{WP-Rechenregel: Iteration}
\begin{description}
...
long multiply(int x, int y) {
	...
	//Nachbedingung S muss gelten
		...
		\end{verbatimtab}
	\item[Nachbedingung] $S = (r = x*y)$
	\item[Invariante] $INV = (a \geq 0 \wedge a*b + r = x*y)$

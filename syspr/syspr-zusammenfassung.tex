...
\end{lstlisting}
\begin{lstlisting}[language=Java, caption=exec() Pseudocode, style=JavaStyle]
algorithm exec
input: (1) file name 
       (2) parameter list
...
copy exec parameters from old address space to system space; for (every region attached to process)
...
\end{lstlisting}
\begin{lstlisting}[language=Java, caption=fork() Pseudocode, style=JavaStyle]
algorithm fork 
input: none 
output: to parent process, child PID number; to child, 0 {
	...
	incr. counts on curr. directory inode and changed root (if appl.);
	increment open 	file counts in file table;
	make copy of parent context (u_area, text, data, stack) in memory;
	push dummy system level context layer on child system level context; 
	...
	if (executing process is parent process) {
		...
		initialize u_area timing fields;
		return (0); /* to user */
	}
}
\end{lstlisting}
...
\begin{lstlisting}[language=Java, caption=exit() Pseudocode, style=JavaStyle]
algorithm exit 
input: return code for parent proc 
output: to parent process, child PID number; to child, 0 {
	ignore all signals;
	...
	close all open files (internal version of algorithm iclose); 
	...
	assign parent proc ID of all child processes to be init proc (1); 
...
\textit{wait()} liefert dem Elternprozess dann den Exit Code des Kind- prozesses zurück, oder einen Indikator, warum der Kindprozess vom Betriebssystem terminiert wurde. \\
...
\subsection{Threads}
\subsubsection{Prozesse versus Threads}
\begin{itemize}
